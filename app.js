...
async function puxa() {
    const recebe = await fetch("https://free-to-play-games-database.p.rapidapi.com/api/game?id=1", {
        "method": "GET",
        "headers": {
    ...
    const recebe2 = await recebe.json()
...
NEXT.addEventListener('click', async function buscarGame() {
    ...
    if (conta < promiseFinal.length - 1) {
        ...
        for (let i = 0; i == promiseFinal[conta].status; i = 0) {
            console.log(promiseFinal[conta].status)
        ...
        console.log(conta)
        console.log(promiseFinal.length - 1)
